...
class Command(BaseCommand):
    ...
    def handle(self, *a, **k):
        ...
        u_val = lambda r: ([] if r[1].strip() and EMAIL_RE.match(r[2]) else
                           (["name empty"] if not r[1].strip() else ["email invalid"]))
        p_val = lambda r: ([] if r[1].strip() and float(r[2]) >= 0 else
                           (["name empty"] if not r[1].strip() else ["price invalid"]))
        def o_val(r):
            errs = []
        ...
        o_create = lambda r: {"legacy_id": r[0], "user_legacy_id": r[1],
                              "product_legacy_id": r[2], "quantity": r[3]}

        for i, r in enumerate(USERS, 1):
            threads.append(threading.Thread(target=do_insert, args=(r, UserModel, u_val, u_create, "USER"), name=f"U-{i}"))
        for i, r in enumerate(PRODUCTS, 1):
            threads.append(threading.Thread(target=do_insert, args=(r, ProductModel, p_val, p_create, "PRODUCT"), name=f"P-{i}"))
        for i, r in enumerate(ORDERS, 1):
            threads.append(threading.Thread(target=do_insert, args=(r, OrderModel, o_val, o_create, "ORDER"), name=f"O-{i}"))
